...
ParallelDFS::ParallelDFS(MPI_Data& mpi_data, TreeSearchData* treesearch_data,
		Log* log, Timer* timer) :
...
bool ParallelDFS::Probe(MPI_Data& mpi_data, TreeSearchData* treesearch_data) {
...
// TODO ParallelDFS
void ParallelDFS::Give(MPI_Data& mpi_data, VariableLengthItemsetStack * st,
		int steal_num) {
	DBG(D(3) << "Give: "
	;
	);
...
void ParallelDFS::ProbeExecute(MPI_Data& mpi_data,
	TreeSearchData* treesearch_data, MPI_Status* probe_status, int probe_src,
	int probe_tag) {
...
for (int i = 0; i < k_echo_tree_branch; i++) {
	...
	DBG(
			D(3) << "SendDTDRequest: dst=" << mpi_data.bcast_targets_[i]
					<< "\ttimezone=" << mpi_data.dtd_->time_zone_ << std::endl
			;
);
}
}
...
mpi_data.dtd_->not_empty_ = !(treesearch_data->node_stack_->Empty())
	...
	|| treesearch_data->stealer_->StealStarted() || mpi_data.processing_node_; // thieves_ and stealer state check
// for Steal2
// dtd_.not_empty_ =
...
DBG(
	D(3) << "SendDTDReply: dst = " << mpi_data.bcast_source_ << "\tcount="
			<< message[0] << "\ttw=" << tw_flag << "\tem=" << em_flag
			<< std::endl
	;
);
...
DBG(
	D(3) << "RecvDTDReply: src=" << src << "\tcount=" << message[0] << "\ttw="
			<< message[1] << "\tem=" << message[2] << "\treduced_count="
			<< mpi_data.dtd_->reduce_count_ << "\treduced_tw="
			<< mpi_data.dtd_->reduce_time_warp_ << "\treduced_em="
...
void ParallelDFS::SendRequest(MPI_Data& mpi_data, int dst, int is_lifeline) {
assert(dst >= 0);
int message[2];
...
DBG(
D(2) << "SendRequest: dst=" << dst << "\tis_lifeline=" << is_lifeline
...
DBG(
D(2) << "SendReject: dst=" << dst << "\tdtd_count=" << mpi_data.dtd_->count_
<< std::endl
;
);
}
...
mpi_data.dtd_->UpdateTimeZone(timezone);
DBG(
D(2) << "RecvReject: src=" << src << "\tdtd_count=" << mpi_data.dtd_->count_
<< std::endl
;
);
...
void ParallelDFS::SendGive(MPI_Data& mpi_data, VariableLengthItemsetStack * st,
...
DBG(
D(2) << "SendGive: " << "\ttimezone=" << mpi_data.dtd_->time_zone_ << "\tdst="

<< dst << "\tlfl=" << is_lifeline << "\tsize=" << size << "\tnode="
<< st->NuItemset() << "\tdtd_count=" << mpi_data.dtd_->count_ << std::endl
;
);
 //st->PrintAll(D(false));
}

void ParallelDFS::RecvGive(MPI_Data& mpi_data, TreeSearchData* treesearch_data,
...
CallRecv(treesearch_data->give_stack_->Stack(), count, MPI_INT, src, Tag::GIVE,
&recv_status);
mpi_data.dtd_->OnRecv();
assert(src == recv_status.MPI_SOURCE);
...
treesearch_data->give_stack_->Stack() + VariableLengthItemsetStack::SENTINEL
+ 1, count - VariableLengthItemsetStack::SENTINEL - 1);
...
DBG(
D(2)  << "RecvGive: src=" << src << "\ttimezone=" << mpi_data.dtd_->time_zone_

<< "\tlfl=" << flag << "\tsize=" << count << "\tnode="
<< (new_nu_itemset - orig_nu_itemset) << "\tdtd_count=" << mpi_data.dtd_->count_
<< std::endl
;
);
...
int ParallelDFS::CallRecv(void * buffer, int data_count, MPI_Datatype type,
...
int ParallelDFS::CallBsend(void * buffer, int data_count, MPI_Datatype type,
int dest, int tag) {
...
int ParallelDFS::CallBcast(void * buffer, int data_count, MPI_Datatype type) {
long long int start_time;
long long int end_time;
log_->d_.bcast_num_++;
start_time = timer_->Elapsed();
